...
@workflow.defn
class SignalQueryVertexWorkflow:
    ...
    async def run(self, inactivity_timeout_minutes: int) -> str:
        while True:
            ...
            while self.prompt_queue:
                ...
                # Send the prompt to Amazon Vertex
    ...
    # Helper method used in prompts to Amazon Vertex
    def format_history(self) -> str:
        return " ".join(f"{text}" for _, text in self.conversation_history)

    # Create the prompt given to Amazon Vertex for each conversational turn
    ...
    # Create the prompt to Amazon Vertex to summarize the conversation history
